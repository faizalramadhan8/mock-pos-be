...
type CashSessionController struct {
	Log     *zerolog.Logger
	Service *usecase.CashSessionService
}

func NewCashSessionController(ctx context.Context) *CashSessionController {
	logger := ctx.Value(enum.LoggerCtxKey).(*zerolog.Logger)
	db := ctx.Value(enum.GormCtxKey).(*gorm.DB)
	return &CashSessionController{
		Log:     logger,
		Service: usecase.NewCashSessionService(ctx, db),
	}
}

func (ctrl *CashSessionController) GetAll(c *fiber.Ctx) error {
	sessions, fail := ctrl.Service.GetAll()
	if fail != nil {
	...
	return c.JSON(dto.ApiResponse{Code: fiber.StatusOK, Message: "successfully", Body: sessions})
}

func (ctrl *CashSessionController) GetOpen(c *fiber.Ctx) error {
	session, fail := ctrl.Service.GetOpen()
	if fail != nil {
		return c.Status(fail.StatusCode.Code).JSON(dto.ApiResponse{Code: fail.StatusCode.Code, Message: fail.StatusCode.Message, Error: fail.Message})
	}
	return c.JSON(dto.ApiResponse{Code: fiber.StatusOK, Message: "successfully", Body: session})
}

func (ctrl *CashSessionController) Open(c *fiber.Ctx) error {
	...
	resp, fail := ctrl.Service.Open(req)
	if fail != nil {
		return c.Status(fail.StatusCode.Code).JSON(dto.ApiResponse{Code: fail.StatusCode.Code, Message: fail.StatusCode.Message, Error: fail.Message})
	}
...
func (ctrl *CashSessionController) Close(c *fiber.Ctx) error {
	id := c.Params("id")
	var req dto.CloseCashSessionRequest
	if err := c.BodyParser(&req); err != nil {
		return c.Status(fiber.StatusUnprocessableEntity).JSON(dto.ApiResponse{Code: fiber.ErrUnprocessableEntity.Code, Message: fiber.ErrUnprocessableEntity.Message, Error: err.Error()})
	}
	if err := util.ValidateRequest(req); err != nil {
	...
	resp, fail := ctrl.Service.Close(id, req)
	if fail != nil {
		return c.Status(fail.StatusCode.Code).JSON(dto.ApiResponse{Code: fail.StatusCode.Code, Message: fail.StatusCode.Message, Error: fail.Message})
	}
